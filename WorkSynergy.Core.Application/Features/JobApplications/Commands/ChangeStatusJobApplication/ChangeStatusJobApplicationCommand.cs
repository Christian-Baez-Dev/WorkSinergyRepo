...
namespace WorkSynergy.Core.Application.Features.JobApplications.Commands.ChangeStatusJobApplication
{
    ...
    public class ChangeStatusJobApplicationCommandHandler : IRequestHandler<ChangeStatusJobApplicationCommand, Response<int>>
    {
        ...
        public async Task<Response<int>> Handle(ChangeStatusJobApplicationCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!Enum.IsDefined(typeof(AsynchronousStatus), request.StatusName) || request.StatusName.Equals(nameof(AsynchronousStatus.Waiting)))
                throw new ApiException("Wrong job application status provided", StatusCodes.Status400BadRequest);
